...
Локальный незадеплоенный Stage 5 consistency slice (2026-08-24) добавляет privacy-safe Telegram profile metadata, human-readable Trello presentation, durable primary-locale fallback, deterministic relative-date/weekend/time-window handling, компактную lifecycle projection, exact team-calendar routing guard, optional public team-calendar embeds и Tailwind-based Demo Console. Он содержит только additive Supabase migrations и требует отдельной review, deploy и production verification; migrations и public calendar settings ещё не применялись. Read-only audit обнаружил старые synthetic Calendar events в personal/primary calendar; они намеренно сохранены, а source-level fix пока не развернут.
