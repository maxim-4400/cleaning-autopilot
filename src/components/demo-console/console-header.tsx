...
export function ConsoleHeader({ active }: { active: "dashboard" | "settings" }) {
  ...
  const brand = <Link aria-label="Sherlock Cleaning dashboard" className="flex min-h-11 items-center gap-3 text-base font-bold text-slate-900 no-underline" href="/dashboard"><Image alt="Sherlock Cleaning" className="rounded-xl object-cover" height={40} src="/sherlock-cleaning-avatar.png" width={40} /><span>Sherlock Cleaning</span></Link>;
  return <>
    <button aria-controls="console-mobile-nav" aria-expanded={open} aria-label="Open navigation" className="fixed left-3 top-3 z-50 grid min-h-11 min-w-11 place-content-center gap-1 rounded-xl border border-slate-200 bg-white shadow-sm md:hidden" onClick={() => setOpen((value) => !value)} ref={buttonRef} type="button"><span className="block h-0.5 w-5 bg-slate-700" /><span className="block h-0.5 w-5 bg-slate-700" /><span className="block h-0.5 w-5 bg-slate-700" /></button>
    <aside aria-label="Console navigation" className="fixed inset-y-0 left-0 z-30 hidden w-[232px] flex-col border-r border-slate-200 bg-white p-4 md:flex">{brand}<nav className="mt-9 grid gap-1">{navigation}</nav><div className="mt-auto grid gap-3 text-sm text-slate-500"><span>admin@sherlockcleaning.rs</span><button className="min-h-11 text-left text-sm font-bold text-teal-800" onClick={() => void logout()} type="button">Log out</button></div></aside>
