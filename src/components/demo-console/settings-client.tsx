...
type Config = { systemPrompt: string; pricingRules: PricingRules; source: "baseline" | "active"; savedAt?: string; revision: string; sections: { prompt: ConfigSectionState; pricing: ConfigSectionState } };
...
function SettingsData({ accessToken }: { accessToken: string }) {
  ...
  return <><ConsoleHeader active="settings" />{!config ? <main aria-live="polite" className="grid min-h-screen place-items-center bg-slate-50 px-6 text-center text-sm text-slate-600 md:ml-[var(--console-sidebar-width)]">{error ?? "Loading saved configuration…"}</main> : <main className="min-h-screen bg-slate-50 pb-16 pt-20 text-slate-900 md:ml-[var(--console-sidebar-width)] md:pt-12"><div className="mx-auto w-full max-w-6xl px-4 sm:px-6 lg:px-8"><section className="mb-8 max-w-3xl"><h1 className="text-4xl font-semibold tracking-tight sm:text-5xl">Agent behaviour and pricing.</h1><p className="mt-4 text-base leading-7 text-slate-600">Update the prompt or pricing separately. The backend validates pricing and writes a single safe active snapshot.</p></section><ConfigurationEditors accessToken={accessToken} pricingRules={config.pricingRules} prompt={config.systemPrompt} savedAt={config.savedAt} sections={config.sections} source={config.source} /></div></main>}</>;
}
...
function isSectionMap(value: unknown): value is Config["sections"] { return typeof value === "object" && value !== null && "prompt" in value && "pricing" in value && isSectionState(value.prompt) && isSectionState(value.pricing); }
